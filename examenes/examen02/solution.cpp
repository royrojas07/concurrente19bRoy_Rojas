...
int main( int argc, char * argv[] ){
	...
	data->image = read_image( data->file );
	...
	data->mirror = get_mirror( data->init_row, data->fin_row );
	...
	if( rank != 0 ){
		...
		int areas_count = cloud_areas.size();
		MPI_Send( &areas_count, 1, MPI::INT, next, 0, MPI_COMM_WORLD );
...
void search_clouds( data_t * data ){
	for( int r = data->init_row; row < data->fin_row; ++r ){
		for( int c = 0; col < data->cols; ++c ){
...
void expand_cloud( int r, int c, data_t * data ){
	if( exists( r, c, data->init_row, data->fin_row ) && data->mirror[r][c] == 0
...
double ** read_image( data_t * data ){
	double ** image = malloc( data->rows * sizeof( double * ) );
	for( int i = 0; i < data->rows; ++i )
		image[i] = malloc( data->cols * sizeof( double ) );
...
int get_mirror( int begin, int end ){
	int ** mirror = malloc( (end - begin) * sizeof( double * ) );
	for( int i = 0; i < end; ++i )
		image[i] = calloc( data->cols, sizeof( double ) );
}
